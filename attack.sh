...
error_check(){
   if [[ $1 == Error* ]]; then
      echo "Error: curl for line $2 in file $3 was not executed. Message: $1" >> errors.log
   fi
}
...
echo "$attacked test cases were successfully attacked."
duration=$(echo "$end - $start" | bc)
echo -e "start: $start\nend: $end\nThis task took $duration milliseconds"
